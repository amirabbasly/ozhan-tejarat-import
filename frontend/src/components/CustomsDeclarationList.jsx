...
import {
  fetchDeclarations,
  saveMultipleDeclarations,
} from "../actions/customsDeclarationActions";
import {
  setCustomsParams,
  clearCustomsParams,
} from "../actions/customsParamsAction";
import "../style/CustomsDeclarationList.css"; // Import the CSS file
...
const CustomsDeclarationList = () => {
  ...
  const handleSaveSelected = () => {
    ...
    dispatch(
      saveMultipleDeclarations(selectedDeclarations, ssdsshGUID, urlVCodeInt)
    );
  };
  ...
  return (
    <div className="customs-declaration-container">
      ...
      <button
        ...
        disabled={
          selectedDeclarations.length === 0 || savingMultipleDeclarations
        }
      >
        {savingMultipleDeclarations
      ...
      {savingMultipleDeclarations && (
        <div className="progress-container">
          ...
          <p>{`در حال ذخیره ${saveMultipleDeclarationsProgress.current || 0}/${
            saveMultipleDeclarationsProgress.total ||
      ...
      {saveMultipleDeclarationsMessage &&
        typeof saveMultipleDeclarationsMessage === "string" && (
          <div>
            ...
            <tbody>
              {declarations.map((item, index) => (
                <tr key={item.FullSerialNumber || index}>
                  ...
                  <td>
                    <Link
                      to={`/declarations/${encodeURIComponent(
                        item.FullSerialNumber
                      )}`}
