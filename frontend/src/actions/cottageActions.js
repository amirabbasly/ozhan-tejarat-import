...
export const fetchCottages =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      if (filters.cottageDateBefore)
        params.append("cottage_date_before", filters.cottageDateBefore);

      if (filters.cottageDate)
        params.append("cottage_date_after", filters.cottageDate);
      if (filters.docs_recieved)
        params.append("docs_recieved", filters.docs_recieved);
      if (filters.rafee_taahod)
        params.append("rafee_taahod", filters.rafee_taahod);
      if (filters.rewatch) params.append("rewatch", filters.rewatch);

      // NEW: add 'search' if it exists
      if (filters.search) params.append("search", filters.search);
      const response = await axiosInstance.get(
        `cottages/?` + params.toString()
      );
...
export const fetchCottageDetails = (cottageNumber) => async (dispatch) => {
  ...
  try {
    const response = await axiosInstance.get(
      `cottages/by-number/${cottageNumber}/`
    );
...
export const updateCottageCurrencyPrice =
  (cottageId, currencyPrice) => async (dispatch) => {
    ...
    try {
      ...
      dispatch({
        type: UPDATE_COTTAGE_CURRENCY_PRICE_SUCCESS,
      ...
      dispatch({
        type: UPDATE_COTTAGE_CURRENCY_PRICE_FAILURE,
        payload: errorMsg,
      });
    }
  };

export const updateCottageDetails =
  (cottageId, updatedCottage, cottageNumber) => async (dispatch) => {
    try {
...
export const deleteExportCottages = (ids) => async (dispatch) => {
  ...
  try {
    const response = await axiosInstance.post(
      "exported-cottage/delete-selected/",
      {
        ids: ids,
      }
    );
...
export const fetchExportCottageDetails =
  (fullSerialNumber) => async (dispatch) => {
    dispatch({ type: FETCH_EXPORT_COTTAGE_DETAILS_REQUEST });

    try {
      const response = await axiosInstance.get(
        `exported-cottage/by-number/${fullSerialNumber}/`
      );
      dispatch({
        type: FETCH_EXPORT_COTTAGE_DETAILS_SUCCESS,
        payload: response.data,
      });
    } catch (error) {
      const errorMsg =
        error.response && error.response.data
          ...
          : error.message;
      dispatch({
        type: FETCH_EXPORT_COTTAGE_DETAILS_FAILURE,
        payload: errorMsg,
      });
    }
  };
export const updateExportCottageDetails =
  (cottageId, updatedCottage, fullSerialNumber) => async (dispatch) => {
    try {
      dispatch({ type: EXPORT_COTTAGE_UPDATE_REQUEST });

      const { data } = await axiosInstance.patch(
        `/exported-cottage/${cottageId}/`,
        updatedCottage
      );
...
export const importCottagesAction = (file) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: IMPORT_EXPORTED_COTTAGES_SUCCESS,
