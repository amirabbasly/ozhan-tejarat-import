...
import {
  ...
  updateOrderStatus,
  deletePerformas,
} from "../actions/performaActions";
import { useLocation, useNavigate } from "react-router-dom";
import "../style/RegedOrderDetails.css";
    ...
    <div className="order-details-container">
      ...
      <div className="order-field">
        ...
        {editMode ? (
          <input
            type="number"
            name="FOB"
            value={formatNumber(formData.FOB) || ""}
            onChange={handleInputChange}
          />
        ) : (
          <span>{formatNumber(order.FOB)}</span>
        )}
      </div>
      <div className="order-field">
        ...
        {editMode ? (
          <input
            ...
            value={formatNumber(formData.prf_freight_price) || ""}
            onChange={handleInputChange}
          />
        ) : (
          <span>{formatNumber(order.prf_freight_price)}</span>
        )}
      </div>
      ...
      <div className="order-field">
        ...
        {editMode ? (
          <input
            ...
            value={formatNumber(formData.prf_total_price) || ""}
            onChange={handleInputChange}
          />
        ) : (
          <span>{formatNumber(order.prf_total_price)}</span>
        )}
      </div>
      ...
      <div className="order-field">
        ...
        {order.remaining_total > -1
          ...
          : `مابع تفاوت : ${formatNumber(-order.remaining_total)}`}
      </div>
      <div className="order-field">
        <label>باقیمانده کرایه حمل:</label>
        <span>{formatNumber(formData.remaining_freight_charges)}</span>
      </div>
      <div className="order-field">
