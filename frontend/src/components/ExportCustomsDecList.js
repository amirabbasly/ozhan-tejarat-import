...
import { fetchExportDeclarations } from '../actions/customsDeclarationActions';
...
const ExportCustomsDecList = () => {
  ...
  }, [exportDeclarations, saveMultipleDeclarationsProgress, failedDeclarations, saveMultipleDeclarationsMessage]);
...
{saveMultipleDeclarationsError && (
  <p className="error">
    {typeof saveMultipleDeclarationsError === 'string'
      ...
      {!loading && !error && exportDeclarations.length === 0 && <p className="no-data">هیچ اظهارنامه‌ای یافت نشد.</p>}
    </div>
  );
};
