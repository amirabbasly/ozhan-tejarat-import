...
export const fetchChecks =
  ({ page = 1, pageSize = 50, search = "", isPaid = "" }) =>
  async (dispatch) => {
    dispatch({ type: FETCH_CHECKS_REQUEST });
    try {
      ...
      const response = await axiosInstance.get(`/checks/?${params.toString()}`);
      console.log("API Response:", response.data); // Debug log

      // Ensure results is an array
      const checks = Array.isArray(response.data.results) ? response.data.results : [];
      ...
      console.error("Fetch Checks Error:", error.response?.data); // Debug log
      dispatch({
        type: FETCH_CHECKS_FAILURE,
        payload: error.response?.data || "Error fetching checks",
      });
    }
  };
...
export const importRepresentation = (file) => async (dispatch) => {
  dispatch({ type: IMPORT_REPRESENTATION_REQUEST });
  const formData = new FormData();
  formData.append("file", file);
  try {
    const response = await axiosInstance.post("/import-representation/", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
    dispatch({
      type: IMPORT_REPRESENTATION_SUCCESS,
      payload: response.data,
    });
  } catch (error) {
    dispatch({
      type: IMPORT_REPRESENTATION_FAILURE,
