...
const CottageForm = () => {
  ...
  useEffect(() => {
    ...
    if (cottageCreation.error) {
      alert(
        "Failed to create cottage: " + cottageCreation.error &&
          cottageCreation.error.total_value
      );
      dispatch({ type: RESET_COTTAGE_CREATION });
    }
  ...
  const handleSelectChange = (selectedOption) => {
    setFormData({
      ...formData,
      proforma: selectedOption ? selectedOption.value : "",
    });
  };
  // Prepare options for the customers select
  const costumerOptions = costumerList.map((customer) => ({
    value: customer.id,
    label: customer.full_name,
  }));

  const handleCostumerSelectChange = (selectedOption) => {
    setFormData({
      ...formData,
      cottage_customer: selectedOption ? selectedOption.value : "",
    });
  };
  ...
  return (
    <form className="cottage-form" onSubmit={handleSubmit}>
      ...
      <div className="form-group">
        ...
        <DatePicker
          ...
          onChange={(date) =>
            setFormData({
              ...formData,
              cottage_date: date.format("YYYY-MM-DD"),
            })
          }
          calendar={persian}
          locale={persian_fa}
        ...
        <Select
          ...
          className="selectPrf"
          value={
            orderOptions.find((option) => option.value === formData.proforma) ||
            null
          }
        ...
        <Select
          ...
          value={
            costumerOptions.find(
              (option) => option.value === formData.cottage_customer
            ) || null
          }
          onChange={handleCostumerSelectChange}
        ...
        <input
          ...
          onChange={(e) =>
            setFormData({
              ...formData,
              rafee_taahod: e.target.checked ? true : false,
            })
          }
        />
        <span>بازبینی</span>
        <input
          ...
          onChange={(e) =>
            setFormData({
              ...formData,
              rewatch: e.target.checked ? true : false,
            })
          }
        />
        ...
        <input
          ...
          onChange={(e) =>
            setFormData({
              ...formData,
              docs_recieved: e.target.checked ? true : false,
            })
          }
        />
        <span>آپلود مدارک</span>
        <input name="documents" type="file" onChange={handleFileChange} />
      </div>
      {/* Goods Section */}
      ...
      <button
        type="submit"
        className="btn-grad1"
        disabled={cottageCreation.loading}
      >
